...
svm.model = svm(income_50k~., data = trainSet, kernel = 'linear', cost = 1, probability = TRUE)
svm.pred = predict(svm.model, testSet, probability = TRUE)
# Confusion Matrix
svm.table = table(svm.pred, testSet$income_50k)
svm.table
#AUC
pred.prob = attr(svm.pred, "probabilities")
pred.to.roc = pred.prob[, 2]
pred.rocr = prediction(pred.to.roc, testSet$income_50k)
perf.rocr = performance(pred.rocr, measure = "auc", x.measure = "cutoff")
no.cross.auc = perf.rocr@y.values[[1]]
perf.tpr.rocr = performance(pred.rocr, "tpr", "fpr")
plot(perf.tpr.rocr, colorize=T, main=paste("AUC:",(perf.rocr@y.values)))
#Other Indicators
evaluation = evaluate.model(svm.table)
...
barp(testdf,main = "SVM Result", ylab = "Value", names.arg = colnames(testdf), col = 2:3)
